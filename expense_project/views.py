...
def dashboard(request):
    ...
    start_today_data = today_date_time.replace(
        hour=0, minute=0, second=0, microsecond=0)
    end_today_data = today_date_time.replace(
        hour=23, minute=59, second=59, microsecond=999999)

    incomes_today_display = Income.objects.filter(user=request.user, created_at__range=(
        start_today_data, end_today_data)).order_by('-created_at')
    expenses_today_display = Expense.objects.filter(user=request.user, created_at__range=(
        start_today_data, end_today_data)).order_by('-created_at')
    total_incomes = Income.objects.filter(user=request.user)
    expenses_year = Expense.objects.filter(
        user=request.user, date__year=today_date.year)
    ...
    earning = total_incomes.aggregate(Sum('amount'))

    spent_year_count = expenses_year.count()
    earning_count = total_incomes.count()
...
def complete_spreadsheet_excel(request):
    ...
    response['Content-Disposition'] = 'attachment; filename=Incomes-Expenses-' + \
        str(request.user.username) + '-' + str(localtime())+".xls"
    ...
    income_list = incomes.values_list(
        'date', 'source__source', 'description', 'amount')
    expense_list = expenses.values_list(
        'date', 'category__name', 'description', 'amount')
    rows = income_list
    ...
    style = xlwt.easyxf(
        'pattern: pattern solid, fore_colour light_blue;''font: colour red, bold True;')
...
def complete_spreadsheet_csv(request):
    ...
    response['Content-Disposition'] = 'attachment; filename=Incomes-Expenses-' + \
        str(request.user.username) + "-" + str(localtime()) + ".csv"

    writer = csv.writer(response)
    writer.writerow(['Date', 'Source', 'Category',
                    'Description', 'Amount In', 'Amount Out'])
    incomes = Income.objects.filter(user=request.user).order_by('date')
    expenses = Expense.objects.filter(user=request.user).order_by('date')
    income_list = incomes.values_list(
    ...
    for income in incomes:
        writer.writerow([income.date, income.source.source,
                        '', income.description, income.amount])
    writer.writerow(['', '', '', ''])

    for expense in expenses:
        writer.writerow([expense.date, '', expense.category.name,
    ...
    writer.writerow(['TOTAL', '', '', '', income_total,
                    expense_total, 'BALANCE', str(balance)])
    return response
...
def complete_spreadsheet_pdf(request):
    ...
    response['Content-Disposition'] = 'inline; attachment; filename=Incomes-Expenses-' + \
        str(request.user.username) + '-' + str(localtime()) + ".pdf"
