...
def dashboard(request):
    ...
    incomes_today_display = Income.objects.filter(
        user=request.user, created_at__range=(start_today_data, end_today_data)
    ).order_by("-created_at")
    expenses_today_display = Expense.objects.filter(
        user=request.user, created_at__range=(start_today_data, end_today_data)
    ).order_by("-created_at")
...
def complete_spreadsheet_excel(request):
    ...
    response["Content-Disposition"] = (
        "attachment; filename=Incomes-Expenses-"
...
def complete_spreadsheet_csv(request):
    ...
    writer.writerow(
        ["Date", "Source", "Category", "Description", "Amount In", "Amount Out"]
    )
    incomes = Income.objects.filter(user=request.user).order_by("date")
    expenses = Expense.objects.filter(user=request.user).order_by("date")
    income_list = incomes.values_list("date", "source__source", "description", "amount")
    ...
    writer.writerow(
        ["TOTAL", "", "", "", income_total, expense_total, "BALANCE", str(balance)]
    )
    return response
...
def complete_spreadsheet_pdf(request):
    ...
    response["Content-Disposition"] = (
        "inline; attachment; filename=Incomes-Expenses-"
        + str(request.user.username)
        + "-"
        + str(localtime())
        + ".pdf"
