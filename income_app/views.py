    ...
    try:
        ...
        if "date_from" in request.GET and request.GET["date_from"] != "":
            ...
            if "date_to" in request.GET and request.GET["date_to"] != "":
                ...
                incomes = incomes.filter(date__gte=date_from).order_by("-date")
            ...
            incomes = incomes.filter(date__lte=date_to).order_by("-date")
...
def add_income_source(request):
    ...
    context = {"sources": sources, "values": request.POST, "create": True}
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "income_app/income_source_import.html",
            {"sources": sources, "create": True},
        )
...
def edit_income_source(request, id):
    ...
    context = {"value": source_obj.source, "update": True, "id": source_obj.id}
    ...
    if request.method == "POST":
        ...
        context = {"value": source, "update": True, "id": source_obj.id}
...
def edit_income(request, id):
    ...
    sources = IncomeSource.objects.filter(user=request.user).exclude(
        id=income.source.id
    )
...
def download_as_excel(request, filter_by):
    ...
    incomes = queryset_filter(
        User.objects.get(username=request.user.username), filter_by
    ).order_by("date")
...
def download_as_csv(request, filter_by):
    ...
    for income in incomes:
        writer.writerow(
            [income.date, income.source.source, income.description, income.amount]
        )

    writer.writerow(["", "", "", ""])
    writer.writerow(
        ["TOTAL", "", "", str(incomes.aggregate(Sum("amount"))["amount__sum"])]
    )
    return response
...
def import_income(request):
    return render(request, "income_app/income_source_import.html", {"upload": True})
...
def upload_csv(request):
    ...
    if request.method == "POST":
        try:
            ...
            if csv.shape[0] > 10:
                messages.error(
                    request, "Please upload a CSV file with less than 10 rows."
                )
                return redirect("import_income")
            ...
            if IncomeSource.objects.filter(user=request.user, source="Loaded From Csv"):
                csv_income_source = IncomeSource.objects.get(
                    user=request.user, source="Loaded From Csv"
                )
            else:
                csv_income_source = IncomeSource.objects.create(
                    user=request.user, source="Loaded From Csv"
                )
            ...
            for i, row in csv.iterrows():
                if not pd.isna(row["date"]):
                    ...
                    try:
                        date = datetime.date(
                            2000 + int(date[2]), int(date[1]), int(date[0])
                        )
                    except:
                ...
                if not pd.isna(row["source"]):
                    ...
                    if IncomeSource.objects.filter(
            ...
            messages.error(
                request, "Please Check if the format of csv file is correct."
            )
            return redirect("import_income")
...
def upload_excel(request):
    ...
    if request.method == "POST":
        try:
            ...
            if not (
                excel_file.name.endswith(".xls") or excel_file.name.endswith(".xlsx")
            ):
            ...
            if len(income_excel_data) > 11:
                messages.error(
                    request, "Please upload a excel file with less than 10 rows."
                )
                return redirect("import_income")

            if IncomeSource.objects.filter(
                user=request.user, source="Loaded From Excel"
            ):
                excel_income_source = IncomeSource.objects.get(
                    user=request.user, source="Loaded From Excel"
                )
            else:
                excel_income_source = IncomeSource.objects.create(
                    user=request.user, source="Loaded From Excel"
                )
            ...
            if headers != ["date", "source", "description", "amount"]:
                ...
                messages.error(
                    request, "Please Check if the format of excel file is correct."
                )
                return redirect("import_income")
