    ...
    try:
        ...
        if "date_from" in request.GET and request.GET["date_from"] != "":
            ...
            if "date_to" in request.GET and request.GET["date_to"] != "":
                ...
                expenses = expenses.filter(date__gte=date_from).order_by("-date")
            ...
            expenses = expenses.filter(date__lte=date_to).order_by("-date")
...
def add_expense_category(request):
    ...
    context = {"categories": categories, "values": request.POST, "create": True}
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "expense_app/expense_category_import.html",
...
def edit_expense_category(request, id):
    ...
    context = {"value": category.name, "update": True, "id": category.id}
    ...
    if request.method == "POST":
        ...
        context = {"value": name, "update": True, "id": category.id}
...
def edit_expense(request, id):
    ...
    categories = ExpenseCategory.objects.filter(user=request.user).exclude(
...
def download_as_excel(request, filter_by):
    ...
    ws.write(
        row_number, 3, str(expenses.aggregate(Sum("amount"))["amount__sum"]), style
    )
    wb.save(response)
    return response
...
def download_as_csv(request, filter_by):
    ...
    writer.writerow(["", "", "", ""])
    writer.writerow(
        ["TOTAL", "", "", str(expenses.aggregate(Sum("amount"))["amount__sum"])]
    )
    return response
...
def import_expense(request):
    return render(request, "expense_app/expense_category_import.html", {"upload": True})
    ...
    if request.method == "POST":
        try:
            ...
            if ExpenseCategory.objects.filter(
                user=request.user, name="Loaded From Csv"
            ):
                csv_expense_category = ExpenseCategory.objects.get(
                    user=request.user, name="Loaded From Csv"
                )
            else:
                csv_expense_category = ExpenseCategory.objects.create(
                    user=request.user, name="Loaded From Csv"
                )
...
def upload_excel(request):
    ...
    if request.method == "POST":
        try:
            ...
            if ExpenseCategory.objects.filter(
                user=request.user, name="Loaded From Excel"
            ):
                excel_expense_category = ExpenseCategory.objects.get(
                    user=request.user, name="Loaded From Excel"
                )
            else:
                excel_expense_category = ExpenseCategory.objects.create(
                    user=request.user, name="Loaded From Excel"
                )
