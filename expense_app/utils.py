...
def queryset_filter(user, filter_by):
    ...
    if filter_by.lower() == "today":
        ...
        expenses = Expense.objects.filter(
            user=user,
            date__year=today_date_time.year,
            date__month=today_date_time.month,
        )
...
def expense_send_success_mail(request, filename, number_of_expenses, type):
    if UserProfile.objects.get(user=request.user).email_preference:
        ...
        extra_info = (
            ...
            + domain
            + ". If you didn't register please contact support team on "
            + domain
        )
        fromEmail = "noreply@income-expense.com"
        html_content = render_to_string(
            "partials/_email_template.html",
            {
        ...
        EmailThread(
            email_subject, text_content, fromEmail, request.user.email, html_content
        ).start()
